...
기준: [`biz-exploration/README.md`](../biz-exploration/README.md)의 회장 리소스 제약(겸업, 사업자등록 불가, 무자본 우선, 계좌 직접입금 우선, 플랫폼 공식 API 자동화 지원 여부가 1차 필터) — 이건 특정 계열사 아이디어가 아니라 회장이 정한 그룹 공통 운영 규칙이라 그대로 적용한다.

**2026-08-11 정정** — Round 1~4는 "나다컴퍼니1이 이미 다루는 도메인은 의도적으로 배제"하는 방식으로 진행했는데, 이건 창립 지시("정연 쪽 결과를 베끼거나 종속되지 않고 독립적으로 발굴")를 잘못 해석한 것이었다. 도메인을 피하려면 `biz-exploration/candidates.md`를 계속 참고해야 하는데, 그 자체가 독립성을 깨는 행위였다. **Round 5부터는 `biz-exploration/candidates.md`를 아예 보지 않고 순수하게 독립적으로 발굴한다** — 겹치는 후보가 나와도 그대로 둔다(오히려 두 관계사가 독립적으로 같은 결론에 도달했다는 신호로 취급).
