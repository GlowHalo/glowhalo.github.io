...
- **정본 문서는 [`hq/가입대기.md`](../가입대기.md)** — 회장이 계열사별로 놓치는 가입 대기 건을 모아둔 곳. 카테고리: "처리 대기(무료)", "🔒 회장 직접 로그인 필요", "후순위(유료)", "별도 트랙(비번 표준화)", "처리 완료". 회장이 "이제 그만 추적해도 된다"고 할 때까지 계속 갱신.
...
- 어제(2026-08-19~20) HQ 세션이 스냅샷 성격의 정리 문서도 만들어둠 — [`hq/계정현황-2026-08-19.md`](../계정현황-2026-08-19.md)(인프라 이전 상세), [`hq/계정통합리스트-2026-08-20.md`](../계정통합리스트-2026-08-20.md)(가입됨/정비중/대기/유료후순위 4분류 + 표준계정 비번 감사). 참고용이고, 실제 작업은 `가입대기.md`에 반영.
