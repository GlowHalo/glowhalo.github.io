...
### 2026-08-26 — 메일함 정기 점검 Routine 신설 (하루 2회)

회장 지시로 `tossneon0@gmail.com` 메일함을 하루 2회(05:30 KST, 16:00 KST) 점검하는 Routine 2개를 이 세션(소율)에 self-bind로 등록함 — `trig_01N3bUVQ8qt6RNkMfkZrhZXi`(05:30), `trig_015C1JmVDJAVjaucdQY75jpr`(16:00).

- 규칙: 회신 불필요한 메일(광고·스팸·이미 해결된 것)은 조용히 읽음 처리, 회신 필요한 메일은 **초안만 정리해서 보고 → 회장 승인 후에만 실제 발송**. 특이사항 없으면(전부 불필요 메일이었거나 안읽음 자체가 없으면) 채팅에 아무것도 남기지 않음(노이즈 방지).
- **⚠️ 확인 필요한 리스크**: `create_trigger` 호출 시 "connectors 파라미터가 이 조직에서 지원 안 됨" + "저장되는 커넥터가 없어 발화 시 Gmail 등 MCP 커넥터 도구 없이 실행될 수 있다"는 경고가 떴다. self-bind(이 세션 자체를 계속 이어가는 방식)라 이미 연결된 Gmail 커넥터가 세션에 그대로 남아있을 가능성이 높다고 보고 일단 등록했지만, **첫 발화(가장 빠르면 오늘 16:00 KST 또는 다음날 05:30 KST) 때 실제로 Gmail 도구를 쓸 수 있는지 확인 필요** — 안 되면 claude.ai Routines UI에서 회장이 직접 만드는 방식으로 전환해야 함(도구 안내 메시지에 나온 대안).
