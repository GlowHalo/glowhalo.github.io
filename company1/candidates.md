...
- **GitHub Marketplace — "Code Review Board" GitHub Action 버전.** 이미 만든 3인 AI 리뷰어 페르소나(Security Skeptic·Reliability Realist·Maintainability Pragmatist) 컨셉과 프롬프트를 그대로 재사용해, PR에 자동으로 코멘트를 다는 GitHub Action으로 만든다. GitHub Marketplace는 리포지토리 메타데이터(`action.yml`) 기반 공식 등록 절차가 있어 브라우저 자동화가 필요 없고, 개인 계정으로 게시 가능(사업자등록 불요). 수익화는 GitHub Sponsors 티어 또는 무료 버전+Gumroad 라이선스 키로 Pro 잠금(A2와 동일 패턴). 타겟도 A1·A2와 동일한 인디해커/솔로 개발자라 신규 시장조사 부담이 적음. **리스크**: GitHub Sponsors는 심사가 있고 승인까지 시간이 걸릴 수 있음 — 이건 다음 라운드에서 CTO 검증 필요.
