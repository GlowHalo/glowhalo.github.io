...
3. `/signup` 폼(이름·이메일·비밀번호·비밀번호 확인) 확인 — 표준 자동화 계정(`tossneon0@gmail.com`, displayName `nadacompany`, 표준 비밀번호)으로 입력·제출. **(2026-08-19 정정 — GlowHalo 전면 개명 이후 displayName은 `nadacompany`가 아니라 `glowhalo`로 써야 한다. 아래 "다음 액션"도 이에 맞게 갱신함.)**
...
**다음 액션(회장 확인 필요)**: [facer.io/signup](https://www.facer.io/signup)에서 직접 1회 가입 요청 — 이메일 `tossneon0@gmail.com`, displayName은 **`glowhalo`**(개명 후 표준값, 옛 `nadacompany` 아님), 표준 비밀번호(금고 `standard_login_password`) 사용 권장(다른 자동화 계정들과 통일). 가입 완료 알려주시면 그 뒤 워치페이스 디자인·게시는 이어서 자동 진행.
