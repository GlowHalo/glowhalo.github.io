...
| `circleheroes_keystore_password` / `circleheroes_release_keystore_base64` | Circle Heroes(GlowHalo 6) Android 릴리스 서명 키스토어 — APK 서명·Amazon Appstore/Google Play 제출용. 절차는 `.claude/skills/app-store-deploy/SKILL.md` 참고. **유출·분실 시 재발급 불가**(같은 키로만 기존 앱 업데이트 가능) — 값 자체는 이미 정상 등록·사용 중이었으나 2026-08-23 문서화 누락 발견해 추가 |
