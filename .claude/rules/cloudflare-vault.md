...
| `RAPIDAPI_PROXY_SECRET` | Link Preview API Worker(`nada-company2-link-preview`)가 `X-RapidAPI-Proxy-Secret` 헤더를 검증하는 값. **2026-08-23 정리 — 예전엔 이 이름과 `rapidapi_proxy_secret_link_preview`(2026-08-20에 RapidAPI Studio에서 발급받은 값)가 각각 따로 등록돼 있어서 혼란이 있었다.** 확인해보니 Worker에는 2026-08-20에 임시로 만든 랜덤값이 배포돼 있었고 실제 RapidAPI가 발급한 값과 달랐던 게 원인 — `rapidapi_proxy_secret_link_preview` 쪽 값으로 이 이름 하나에 통합하고 중복 항목은 삭제, Worker도 이 값으로 재배포함. **⚠️ 그런데도 여전히 RapidAPI 게이트웨이 경유 `/v1/preview` 호출이 401** — 즉 이 값 자체가 지금 RapidAPI Hub의 Security 탭에 반영 안 돼 있는 것으로 보인다(2026-08-20 세션이 값만 발급받고 실제로 Security 탭에 저장은 못 했을 가능성, 또는 그 뒤 로그인이 다시 막혔다 풀리는 과정에서 설정이 안 남았을 가능성). **회장이 로그인 다시 가능해진 지금, RapidAPI Hub → GlowHalo Link Preview API → Security 탭에서 이 값이 실제로 저장돼 있는지 직접 확인·저장해야 한다** |
