...
| `glowhalo_org_company_email` | `help@glowhalo.org` — Cloudflare Email Routing으로 `google_login_email`(tossneon0)에 포워딩(2026-08-25 API로 설정, glowhalo.org 존에 Email Routing 신규 활성화 + 규칙 등록). "회사 이메일" 요구하는 가입 폼에 쓸 것 — 개인 Gmail 주소로 막히는 가입에 대응. **옛 키 `nadagroup_org_company_email`(`help@nadagroup.org`)은 삭제됨** — nadagroup.org 환불 실패로 폐기 확정(2026-08-20 공지) 후 이 키로 대체, 옛 주소의 포워딩 규칙도 비활성화 처리(도메인 자체는 남은 계약기간 동안 살아있으니 필요하면 막 테스트용으로만 써도 됨, 실제 계정 가입엔 쓰지 말 것) |
| `browserbase_api_key` / `browserbase_project_id` | Browserbase(클라우드 원격 브라우저) — 이 세션 프록시를 안 거치는 헤드리스 브라우저 자동화용 (무료 플랜). `project_id`는 API 키와 한 쌍으로 SDK 초기화에 필요 — 2026-08-23 문서화 누락 발견해 추가(값 자체는 이미 정상 등록·사용 중이었음) |
...
| `company3_moonshot_state` | 위와 같은 성격의 예외 항목 — GlowHalo 3의 별도 트랙(문샷 전략) 운영 상태 JSON. `execution/moonshot.py`가 자동으로 읽고 쓴다. 2026-08-23 문서화 누락 발견해 추가(값 자체는 이미 정상 등록·사용 중이었음) |
...
| `RAPIDAPI_PROXY_SECRET` | Link Preview API Worker(`glowhalo2-link-preview`)가 `X-RapidAPI-Proxy-Secret` 헤더를 검증하던 값. **⚠️ 2026-08-23 저녁 — 검증 로직 자체를 회장 지시로 해제함(값은 그대로 남아있지만 Worker가 더 이상 이 값을 확인하지 않음).** Zyla API Hub 등 RapidAPI 밖 마켓플레이스 트래픽도 통과시키려고 `verifyRapidApiSecret`이 항상 `true`를 반환하도록 변경·재배포 — 지금은 `/v1/preview`가 헤더 없이도 완전히 열려있다(상세: `niche-api/products/link-preview-api/README.md`). **이 저장소가 공개라 워커 URL이 이미 노출돼 있어 사실상 무인증 공개 API가 된 상태** — RapidAPI에 유료 구독자가 생기면 재검토(Zyla 전용 별도 시크릿 발급 등). 그 전까지의 이력: 예전엔 이 이름과 `rapidapi_proxy_secret_link_preview`가 각각 따로 등록돼 있던 걸(Worker엔 2026-08-20 임시 랜덤값이 배포돼 있었음) 하나로 통합했고, 이 값 자체는 RapidAPI Hub → 해당 API → Gateway 탭 → Firewall Settings에서 확인한 실제 발급값과 일치 확인됨(지금은 검증에 안 쓰일 뿐 값 자체는 유효) |
...
| `cloudflare_login_email` / `cloudflare_login_password` | Cloudflare 대시보드 로그인(표준계정, 2026-08-19 신규 등록) — `cloudflare_api_token`(API 인증용)과는 별개, 대시보드 화면에 직접 로그인해야 할 때 사용. 2026-08-23 문서화 누락 발견해 추가(값 자체는 이미 정상 등록·사용 중이었음) |
...
| `amazon_appstore_shared_key` | Amazon Appstore IAP(인앱결제) 검증용 Shared Key, 2026-08-19 콘솔에서 발견해 등록 — GlowHalo 6 Circle Heroes 수익화용, 실제 IAP 붙일 때 사용. 2026-08-23 문서화 누락 발견해 추가(값 자체는 이미 정상 등록·사용 중이었음) |
| `circleheroes_keystore_password` / `circleheroes_release_keystore_base64` | Circle Heroes(GlowHalo 6) Android 릴리스 서명 키스토어 — APK 서명·Amazon Appstore/Google Play 제출용. 절차는 `.claude/skills/app-store-deploy/SKILL.md` 참고. `circleheroes_keystore_password`는 단일 문자열이 아니라 **JSON**(`alias`/`storePassword`/`keyPassword`/`keystoreType`/`note`)이니 파싱해서 쓸 것. **⚠️ 2026-08-25 GlowHalo 명의로 재발급됨** — 최초 생성분(2026-08-17)은 인증서 소유자가 `OU=nadagroup, O=nadagroup`이라 옛 브랜드가 영구히 박히는 문제가 있었다. **서명키는 앱을 한 번이라도 스토어에 올리면 절대 교체할 수 없어서**(스토어가 서명 불일치를 거부) 아직 미배포인 지금이 마지막 기회였고, 회장 지시로 `CN=Circle Heroes, OU=GlowHalo, O=GlowHalo, L=Seoul, ST=Seoul, C=KR`로 새로 만들어 이 두 항목을 덮어썼다(PKCS12, alias `circleheroes`, 2054년까지 유효, 금고 왕복 후 해시 대조까지 검증). 옛(nadagroup) 키스토어는 회장 지시로 **2026-08-25 금고에서 완전히 삭제**했다(미배포라 쓸 일이 없고, 남겨두면 잘못된 키로 서명하는 사고 위험만 남기 때문). **비밀번호는 회장 지시로 표준 비밀번호(`standard_login_password`와 동일 값)로 통일**했다 — `keytool -storepasswd`로 바꿨고 인증서 지문(SHA256 `58:31:CB:55:…`)이 그대로임을 확인했으므로 **서명 정체성은 변하지 않았다.** 참고로 **비밀번호는 배포 후에도 바꿀 수 있다**(`keytool -storepasswd`, 키 자체는 그대로) — 영구히 고정되는 건 키(=인증서)이지 비밀번호가 아니다. 그래서 표준 비밀번호가 갱신되면 이 키스토어도 같이 바꿔주면 된다. **유출·분실 시 재발급 불가**(같은 키로만 기존 앱 업데이트 가능) |
...
| `discord_webhook_url` | **실사용 중, GlowHalo 7(도현) 쿠팡딜봇 전용.** 디스코드 "나다특가" 서버 `#특가-딜` 채널 게시용 웹훅 — `glowhalo7-coupang-dealbot` Worker에 시크릿으로 이미 등록돼 실제 게시 중(연결·테스트 완료, 상세: [`coupang-partners/products/coupang-dealbot/README.md`](../../coupang-partners/products/coupang-dealbot/README.md)). ⚠️ **`pixel-ai-office/worker/.dev.vars.example`에도 동일한 이름의 환경변수(`DISCORD_WEBHOOK_URL`)가 나오는데, pixel-ai-office는 실배포 없는 참고용 레퍼런스 프로젝트**([`hq/프로젝트점검-예정.md`](../../hq/프로젝트점검-예정.md) 참고, 2026-08-13 "레퍼런스로 재분류, 상품화 계획 없음")**라 이 값과 무관 — 그 프로젝트 스캐폴드 코드가 값을 채워 넣지 않은 빈 플레이스홀더일 뿐, 이 금고 값이 그 쪽에 실제로 연결·사용된 적 없음.** 2026-08-23 계정원장에 "pixel-ai-office 전용"으로 잘못 기재돼 있던 걸 회장이 지적해 바로잡음 |
