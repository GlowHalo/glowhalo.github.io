...
| `RAPIDAPI_PROXY_SECRET` | Link Preview API Worker(`nada-company2-link-preview`)가 `X-RapidAPI-Proxy-Secret` 헤더를 검증하는 값. **✅ 2026-08-23 완전 해결.** 예전엔 이 이름과 `rapidapi_proxy_secret_link_preview`가 각각 따로 등록돼 있던 걸(Worker엔 2026-08-20 임시 랜덤값이 배포돼 있었음) 하나로 통합하고 Worker에 재배포. 이 값은 **RapidAPI가 자동 발급한 값을 그대로 복사해온 것**(RapidAPI Hub → 해당 API → **Gateway 탭 → Firewall Settings**에서 확인 가능 — Security 탭 아님, provider가 직접 값을 입력하는 게 아니라 이미 발급된 값을 복사만 하면 됨) — 회장이 그 화면에서 값을 복사해 대조해준 결과 이미 일치했고, 게이트웨이 경유 `/v1/preview` 실호출도 성공(HTTP 200) 확인 완료 |
...
**⚠️ 2026-08-23 위 정책 재검토 중 — 실행 보류, 다음 세션도 그대로 따를 것.** 소율(비서실) 세션이
이 Notion 페이지를 실제로 열어보니 표준 비밀번호(`xhtmspdhs1!A` 등)가 20여 개 서비스에 **평문으로
재사용**돼 있고, 같은 페이지 하단에 **은행 정산계좌 실번호 + 회장 실명**까지 있어 "유출돼도
크리티컬하지 않다"는 전제와 실제로 어긋난다고 보고 → 회장이 재정비 방향을 확정:
- **지금부터 계정원장(`account-ledger.html` 아티팩트, https://claude.ai/code/artifact/a0f2fb08-bd6b-4138-a675-769c7a665674)이 계정 상태의 최신 정본이다** — 계정별 "정비 완료"(위 체크리스트 충족) 시점마다 그 계정의 최종값을
  **금고**(예: 오늘 RapidAPI 비밀번호를 표준값 `xhtmspdhs1!A`로 교체 → `rapidapi_login_password` 갱신 완료)에
  반영한다. 이 개별 갱신은 지금부터 바로 진행.
- **Notion 페이지 자체는 손대지 않고 그대로 둔다** — 전 계정 정비가 다 끝나면 그때 계정원장 내용을
  기준으로 페이지를 통째로 다시 만든다. 재정비판에서는 실제 비밀번호 문자열을 그대로 적지 않고,
  표준 비밀번호를 쓰는 계정은 **"표준비밀번호"라는 이름표만 적고 값은 안 적는 방식**(값이 필요하면
  금고에서 조회)으로 바꾼다 — 이러면 이 페이지 하나가 유출돼도 개별 값이 새지 않는다. 표준값이 아닌
  계정(RapidAPI처럼 예외적으로 강력한 고유 비밀번호를 쓰는 경우 등)은 이 페이지 자체에 값을 적지
  않고 금고 참조로만 남긴다.
- **재정비 시점까지는 위 "2026-08-18 정책"(Notion에 실제 로그인 값 그대로 적기)을 신규 계정에도
  적용하지 않는다** — 그 정책은 이번 재정비로 대체될 예정이므로, 새 계정이 생겨도 지금은 금고에만
  등록하고 이 Notion 페이지는 건드리지 않는다.
- 트리거: "이제 재정비 시작해" 또는 이와 동등한 회장의 명시적 신호. 그 전엔 이 항목을 다시 꺼내
  묻지 말고 조용히 대기.
