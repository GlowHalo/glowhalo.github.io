...
## 세션별로만 켜고 끄기 — 안 된다 (2026-08-18 조사)

회장 질문: "상시로 필요없는 건 꺼뒀다가 필요할 때만 켜면 어떨까? 설치를 전체가 아니라 필요한 세션에서만 하는 방법은?" 조사 결과:

- **플러그인(`enabledPlugins`)은 지금 이 모노레포 구조에서 세션별/하위폴더별로 나눠 켜는 기능이 없다.** `.claude/settings.json`의 `enabledPlugins`는 `project` 스코프 = 이 저장소를 연 세션 전부에 적용되고, 하위 폴더(`niche-templates/.claude/settings.json` 등)에 따로 둬도 프로젝트 스코프 plugin 설정은 안 먹는다 — Claude Code 자체에 아직 없는 기능이다(`anthropics/claude-code` GitHub 이슈 #62174, "Support per-project enabledPlugins", 2026-05 오픈, 아직 미해결). 즉 "정연 세션엔 cloudflare 플러그인 켜고 채원 세션엔 끄기" 같은 건 지금 못 한다.
  - 수동으로 껐다 켰다(`claude plugin uninstall`/`install --scope project`)는 기술적으로 되지만, `project` 스코프라 **이 저장소를 동시에 열고 있는 다른 계열사 세션 전부**에 즉시 영향을 준다 — 내가 잠깐 쓰고 끄는 사이 다른 세션이 그 플러그인을 잃는다. 여러 세션이 상시 동시 작업 중인 이 저장소 구조에선 위험한 방식이라 권장하지 않는다.
  - 다행히 위 재실측처럼 지금 켜진 5개 합계가 ~3,530 토큰이라 토글의 실익 자체가 작다. 정말 무거운 플러그인(예: 옛 `posthog` 25,534토큰급)을 켤 일이 생기면, 그때는 "보류 목록"처럼 필요한 시점에만 잠깐 설치하고 작업 끝나면 바로 uninstall하는 게 맞다 — 상시 on/off 자동화보다 그때그때 수동 판단.
- **스킬은 이미 하위폴더 단위로 나뉜다** — 위 표(`.claude/skills/` 저장소 전체 vs `<하위폴더>/.claude/skills/`)가 그 답. `product-idea-mining`·`lead-magnet-pdf`는 여러 계열사(정연·채원·지호)가 같이 쓰므로 저장소 루트에 둔 게 맞고, 특정 계열사 전용 스킬이 생기면 그 폴더 밑에 두면 된다.
...
**결론**: 지금 구조(플러그인 5개 project 스코프 + 스킬 2단계 스코프 + 커넥터 계정단위·지연로딩)가 이미 이 저장소가 낼 수 있는 최적에 가깝다. 세션별 플러그인 스코핑은 Claude Code 자체 기능이 나와야 가능(#62174 추적), 그 전까진 "무거운 플러그인만 필요할 때 수동 설치/제거"가 현실적 대안이다.
