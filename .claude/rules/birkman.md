...
- 버크만 사이트 로그인 비밀번호는 원칙적으로 **사용자만 입력**한다.
  - **2026-08-10 예외 — GlowHalo 4(채원)에 한해 로그인 자체는 허용.** 회장이 계정(`tossneon`)을 직접 지정해 채원의 로그인 사용을 승인 — 금고에 `birkman_login_id`/`birkman_login_password`로 등록됨 (`.claude/rules/cloudflare-vault.md` 참고). **이 예외는 로그인 단계에만 적용되고, 구매/발송 확인 게이트는 그대로 유지된다.** 다른 계열사·범용 자동화 목적으로 이 계정을 재사용하지 않는다 — "강하게 관리할 것"이라는 회장 지시가 있었음.
...
- 다운로드 클릭 시 네이티브 저장 다이얼로그가 떠서 스크린샷이 멈출 수 있다(정상). 앱 내장 브라우저에서는 다이얼로그 제어가 안 되므로 사용자가 저장한 뒤 파일을 `data/` 로 옮긴다.
  - **2026-08-17 해법 확인 — 실제 Playwright(CDP 원격 브라우저) 경유일 땐 이 문제 자체가 없다.** 앱 내장 브라우저(`mcp__Claude_Browser__`) 한정 문제였고, `chromium.connectOverCDP()`로 붙인 진짜 Playwright 세션은 `page.waitForEvent('download')`로 다운로드를 정상적으로 코드에서 받아 저장할 수 있다(네이티브 다이얼로그 자체가 안 뜸). 아래 "재정찰" 스크립트 참고 — 다만 이번엔 목록 화면에 다운로드 링크가 없어 실제 다운로드까지는 검증 못함(주문 상세 모달까지 드릴다운 필요, 다음 재정찰 때 이어서).

## 마이페이지 재정찰 (2026-08-17, 채원 — `src/recon-mypage.mjs`)

- **로그인 자동화 성공.** Cloudflare Browser Rendering(메인 경로)으로 `chromium.connectOverCDP()` 접속 → 로그인 → `/mypage/assessment` 리다이렉트의 비밀번호 재확인까지 스크립트 하나로 전부 자동 처리됨. Browserbase(백업)는 이번 확인 시점 무료 한도 소진(402)이었으나 메인 경로로 문제없이 대체됨.
- **실제 과거 주문 이력이 있는 계정임을 확인** — 진단 상품화 이전부터 회장이 실사용해온 디브리퍼 계정. 개인정보(고객사명 등)가 있어 이 문서엔 상세를 남기지 않음, 필요시 회장과 채팅으로만 공유.
- **"진단결과 공유" 컬럼이 전부 "전송"으로 표시됨** — 기존 완료된 주문 전부 이 상태였음. **결과 리포트가 버크만 측에서 대상자에게 자동으로 전달(전송)된다는 가설을 뒷받침하는 정황 증거.** 다만 이게 "이메일 자동발송"을 직접 의미하는지, 다른 방식(예: 마이페이지 접근 권한 부여)인지는 문구만으론 100% 확정 못함 — 실제 신규 주문 1건을 진행해보며 이메일 수신을 직접 확인하는 게 가장 확실.
- **신규 발견 — "모바일 진단안내" 컬럼에 "알림톡 신청/미신청" 옵션이 있음.** 진단 응시 초대가 이메일뿐 아니라 **카카오톡 알림톡**으로도 갈 수 있는 선택지가 있는 것으로 보임(주문 시 신청 여부 선택). 자동화 설계에서 이 옵션을 켤지 끌지도 결정 필요 — 알림톡은 카카오 비즈니스 계정 연동이 별도로 필요할 수 있어 추가 조사 필요.
- **PDF 다운로드 실제 검증은 다음 단계로 남음** — 목록 화면엔 다운로드 링크가 없었고(과거 완료 주문이라 상세 모달 진입이 필요), 실제 대상자 상세를 열어 다운로드 링크까지 드릴다운하는 건 다음 재정찰 때 이어서 확인.
