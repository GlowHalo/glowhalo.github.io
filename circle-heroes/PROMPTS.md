...
### 영웅 화면 — **1~3, 5, 7~9, 11 구현 완료(2026-07-28) / 4, 6, 10은 보류**
...
6. **보류(실제 캡쳐 이미지 없이는 정확한 비율 산정 불가).** 전투 화면 유닛 크기·배치 간격을 캡쳐
   비율로 맞추는 작업 — 저는 캡쳐를 직접 보지 못하고 텍스트 설명만 전달받는 상태라, "몇 %로
   좁혀라" 식의 정확한 수치화가 불가능함. 캡쳐를 다시 첨부해주시면 그 자리에서 진행 가능.
...
10. **보류(블로커 그대로).** 영웅 초상화 클릭 시 상세화면 이동 여부 — 원본 레퍼런스 영상 위치를
    여전히 못 찾음(이 리포 git 이력엔 영상 없음). 위치 확인되면 프레임 분석 진행.
