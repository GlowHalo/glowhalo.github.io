...
### 뽑기(소환) 화면 — **1, 2 구현 완료(2026-07-29) / 3은 별도 설계 필요**
...
3. **보류(시스템 재설계 필요, 미착수).** 뽑기를 **배너(페이지) 선택형**으로 전환 — 배너 3개,
   배너마다 SSR 캐릭터 1명을 픽업으로 배치, 천장 채우면 그 캐릭터 확정 지급. 천장 누적치 표시
   UI도 다듬을 것.
   → 현재 `systems/gacha.ts`는 배너 개념이 없는 단일 확률표 구조라 **시스템 자체를 배너별
   확률표/배너별 천장 카운터로 재설계해야 함** (UI만의 문제가 아님). 진행 여부는 주인님 결정
   필요 — 세이브 데이터 마이그레이션(기존 천장 카운터 처리)도 같이 고려해야 함.
