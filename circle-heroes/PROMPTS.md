...
3. **완료.** 뽑기를 **배너(페이지) 선택형**으로 전환 — 배너 3개, 배너마다 SSR 캐릭터 1명을
   픽업으로 배치, 천장 채우면 그 캐릭터 확정 지급. 천장 누적치 표시 UI도 다듬을 것.
   → `systems/gacha.ts` 전면 재설계. **배너 3개**(발록=불·딜러 / 데스나이트=어둠·탱커 /
   소교=물·서포터, 진영·클래스 다양하게 선정): 일반 SSR 롤 중 50%는 그 배너의 픽업으로 우선
   배정("50/50" 방식), **배너 천장**(`bannerPity[bannerId]`, 50회) 도달 시 다음 뽑기는 그 배너
   픽업 확정 + 카운터 리셋. **기존 전체 천장(UR, 50회)은 그대로 유지** — 배너와 무관하게 전체
   뽑기 횟수로 누적, UR 확정 지급. 즉 두 천장이 독립적으로 동시에 돌아감: "이 배너에서 50번
   안에 픽업 확정" + "전체 50번 안에 UR 확정" — 세이브 마이그레이션은 `save.ts`의 기존
   `{...DEFAULTS, ...parsed}` 병합 로직이 그대로 처리(구버전 세이브에 없는 `bannerPity` 필드는
   자동으로 빈 객체로 채워짐, 기존 `pity` 필드는 하위호환용으로 유지하되 더는 안 씀).
