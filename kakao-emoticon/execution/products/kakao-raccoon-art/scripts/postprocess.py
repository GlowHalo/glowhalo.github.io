...
def chroma_key(im_rgb):
    ...
    h, w, _ = arr.shape
    # [A14 raccoon, 2026-08-19 발견] 32종 중 2종(17, 24)에서 코너 바로 끝 1~4px 안쪽에
    # 흰색 둥근 카드 테두리 아티팩트가 섞여 나옴(A12의 "카드형 배지" 결함과 동일 계열,
    # 프롬프트에 "코너"를 언급 안 해도 낮은 확률로 재발함). offset=2는 이 얇은 흰 테두리를
    # 그대로 샘플링해버려 배경색 추정 자체가 깨짐 — offset을 10으로 늘리고 4코너 중앙값을
    # 써서(평균 대신) 코너 1~2개가 오염돼도 나머지 다수 코너로 안전하게 배경색을 추정한다.
    off = 10
    corners = np.array([arr[off, off], arr[off, w - 1 - off], arr[h - 1 - off, off], arr[h - 1 - off, w - 1 - off]])
