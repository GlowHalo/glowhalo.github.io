...
## 🗓️ 매일 생산 방침 (2026-08-16 회장 확정)

**"매일 레오나르도 1개 + 예슬이 직접 그린 것(SVG) 1개"** 페이스로 신규 시리즈를 만든다 — 하루 2종 고정. 배경: 하루 10개 같은 과속은 (1) 카카오 어뷰징 인상 (2) 로그인 이상탐지 재발 (3) 레오나르도 토큰 고갈(시리즈당 약 35회 API 호출·175토큰 소모, 하루 10개면 1~2일 내 소진) 위험이 있어서 회장과 상의 후 하루 2개로 합의(2026-08-16 대화 기록 참고). 아직 시장 신호(일잘봇 승인·판매 데이터)가 없는 상태라 "양을 늘려서 뭐가 통하는지 보는" 탐색 목적의 페이스. 레오나르도 API 토큰은 회장이 $10 충전 완료(2026-08-16).
