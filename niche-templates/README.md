...
8. **일일 생산 10건 + 업로드 10건 루틴화 (2026-08-12 회장 지시로 확정, 2026-08-16 새벽 4시로 이동 + 무조건 실행으로 강화).** A1 노션 템플릿 라인을 매일 **10건 콘텐츠 생산 + 10건 Gumroad 업로드**(Gumroad 계정 자체의 하루 상품 생성 한도와 동일한 페이스)로 상시 가동한다. 매일 **04:00 KST**에 자동 실행되는 Routine(`trig_01RiNpMgm9xd6TouN7SJBSQK`)이 담당.
   - **2026-08-16 변경 — "대기열 있으면 생산 스킵" 조건부 로직을 폐지, 매일 무조건 10건 신규 생산으로 강화.** 기존엔 업로드 대기열이 10건 밑으로 떨어질 때만 신규 콘텐츠를 만들었으나, 회장이 "매일 새벽 4시 템플릿 10개를 생성하여 판매가능한 상태로 업로드"로 명시적으로 재지시 — 대기열 상태와 무관하게 매일 10건을 새로 기획·제작·발행한다.
   - **품질 기준(15개 프롬프트가 실제로 반박하도록 설계됐는지 등)은 물량을 위해 낮추지 않는다** — 이 원칙은 그대로 유지. **안전장치(2026-08-16 사장 제안, 회장 이견 없어 채택)**: 매일 무조건 10건을 강제하면 언젠가 진짜 차별화된 신규 오디언스가 고갈될 수 있다 — 그날 정직하게 10개를 못 채우면 억지로 채우지 말고, 부족분만큼 **기존 상품 품질 개선/퍼널 점검**(운영원칙 7의 (2)(3))으로 대체하고 이유를 로그에 남긴다.
