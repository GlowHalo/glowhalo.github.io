...
**회장 확인 완료(2026-08-15, 채팅으로 답변 받음)**: 이 세션(Notion MCP)이 물려 있는 "🏪 상품 허브"(나다컴퍼니 > 신사업 탐색)는 **회장이 새로 만든 워크스페이스**이고, 기존 자료(구 워크스페이스, `tossneon0`/`fearless-frog-802.notion.site` 추정)를 여기로 옮기는 중 — "자리잡으면 기존거 지울게"라고 확정. 즉 혼란이 아니라 **진행 중인 정식 이관**이었다. 앞으로 A1 신규 콘텐츠는 이 새 워크스페이스(이 세션이 연결된 곳)를 기준으로 만들면 된다.

**⚠️ 후속 추적 필요 — 구 워크스페이스 삭제 전 필수 확인**: 현재 라이브 중인 22개 Gumroad 상품의 `custom_receipt`(템플릿 복제 링크)는 전부 **구 워크스페이스**(`fearless-frog-802.notion.site`) URL을 가리키고 있다. 회장이 "자리잡으면" 구 워크스페이스를 지우기 전에, (1) 새 워크스페이스의 해당 페이지들이 실제로 "웹에 공개" 상태인지 확인하고 (2) 22개 상품의 receipt 링크를 새 워크스페이스 공개 URL로 전부 교체해야 한다 — 안 하면 이미 구매(향후 매출 발생 시)한 고객이 복제 링크를 못 열게 된다. 삭제 시점이 다가오면 이 항목부터 처리할 것.

**✅ 노션 구조 정리 — 외부공개 전용 최상위 페이지 신설 (2026-08-16, 회장 지시로 실행)**: 처음엔 "🏪 상품 허브"를 그대로 워크스페이스 최상위로 옮겼으나, 회장이 재지시해 **"🌐 나다컴퍼니(외부공개)"라는 신규 최상위 페이지**(page_id `3bedea9b558f81c3a941fb1f652b467a`)를 만들고 그 밑으로 "🏪 상품 허브"(page_id `3bbdea9b558f81328a27dafff176c43a`, 26개 상품 전부 하위)를 다시 이동했다 — 최종 구조: `나다컴퍼니(외부공개)` → `상품 허브` → 상품 26개. 앞으로 A1 외에 다른 계열사도 외부공개 콘텐츠가 생기면 이 "나다컴퍼니(외부공개)" 밑에 형제 페이지로 추가하면 된다(예: "상품 허브"와 나란히 다른 계열사 전용 공개 페이지). **남은 건 회장이 최상위 페이지에 Publish 1클릭뿐** — https://app.notion.com/p/3bedea9b558f81c3a941fb1f652b467a . 이후로 내부 전략·재무·회의록 등은 절대 이 페이지 밑에 두지 않는다(반대 방향 실수 — 안 새야 할 게 새는 것 — 주의).

**🔴 2026-08-16 갱신 — 위 확인, "안 됨"으로 확정. 구 워크스페이스 삭제하면 안 됨.** 신규 검증법(구글/페이스북 크롤러 봇 User-Agent로 `app.notion.com/p/<id>` 요청 → 서버가 공개 페이지는 301로 실제 `notion.site` URL을 알려주고 비공개 페이지는 404 반환)으로 새 워크스페이스의 26개 상품 페이지(기존 21종 + 니치 2차 5종) 전수 확인한 결과 **전부 404, 단 하나도 공개 상태가 아니다**(구 워크스페이스 대조군 4개는 전부 301로 정상 확인됨). "이관 완료"는 콘텐츠 복제 기준으로는 맞지만 Notion의 "웹에 공개" 토글은 페이지 속성이라 복제 과정에 안 딸려왔다. **API/MCP 어디에도 이 토글을 켤 방법이 없어 회장의 실제 클릭 1회가 꼭 필요함** — "🏪 상품 허브" 페이지(https://app.notion.com/p/3bbdea9b558f81328a27dafff176c43a) 하나만 Share→Publish 하면 하위 26개 전부 자동 공개 전환(Notion 공식 정책). **그 전까지는 구 워크스페이스를 절대 삭제하지 말 것** — 지금 지우면 라이브 22개 상품의 receipt가 즉시 깨진다. 완료되면 알려주면 즉시 이어서 21개 receipt 교체 + 니치 2차 5종 발행 가능. 상세: [A1-gumroad-대량생산-자동화.md의 2026-08-16 후속](execution/A1-gumroad-대량생산-자동화.md)·[products/14](execution/products/14-니치마켓-2차.md).
