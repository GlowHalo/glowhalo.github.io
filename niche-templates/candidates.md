...
**✅ 노션 구조 정리 — 외부공개용 최상위 페이지 분리 (2026-08-16, 회장 지시로 실행)**: "🏪 상품 허브"(page_id `3bbdea9b558f81328a27dafff176c43a`, 26개 상품 전부 하위)를 "나다컴퍼니 > 신사업 탐색" 3단계 아래에서 **워크스페이스 최상위로 이동 완료**(`notion-move-pages`로 실행) — 이제 이 페이지가 이 워크스페이스의 유일한 "외부공개용" 루트다. 페이지 본문도 낡은 안내문("AI Board of Directors만 판매중" 등)을 실제 현황(21종 발행+5종 대기)으로 갱신했다. 이 구조를 쓰는 이유: 부모 페이지 하나에만 "웹에 공개"를 켜두면 앞으로 추가하는 상품은 전부 자동 상속되어, 오늘 겪은 "이관해도 공개 토글은 안 따라옴" 문제가 구조적으로 재발하지 않는다. **남은 건 회장이 이 페이지에 Publish 1클릭뿐** — https://app.notion.com/p/3bbdea9b558f81328a27dafff176c43a . 이후로 내부 전략·재무·회의록 등은 절대 이 페이지 밑에 두지 않는다(반대 방향 실수 — 안 새야 할 게 새는 것 — 주의).
