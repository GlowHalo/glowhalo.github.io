...
이 세션에 연결된 Notion MCP가 이 상품을 실제로 만든 계정(`tossneon0`, `fearless-frog-802.notion.site`)이 아니라 **다른 워크스페이스**("나다컴퍼니 > 신사업 탐색 > 🏪 상품 허브", 회장 개인 계정으로 추정)에 물려 있다는 걸 확인했다. 그 워크스페이스 안에도 Investor Panel·Code Review Board를 포함한 20종 템플릿이 **전부 완성된 상태로 별도로 존재**(허브 페이지 자체의 안내 문구는 "1종만 완성, 나머지는 초기상태"라고 돼 있는데 실제로 열어보면 다연 것과 반대 — 안내 문구 자체가 낡은 상태). `candidates.md`의 2026-08-13 "Investor Panel 재발견" 항목은 사실 이 중복 사본을 가리킨 것이었고, 실제로는 이미 2026-08-07부터 라이브인 상품과 같은 컨셉 — 새 기회가 아니라 **워크스페이스 정리 이슈**로 재분류함. 처리 방향(중복 사본 보관/삭제, 두 워크스페이스 연결 정리)은 회장 판단 필요 — `candidates.md` 하단 "회장 확인 요청" 참고.
