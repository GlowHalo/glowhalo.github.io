...
## 기존상품 정비 — 커버 2장→3장 (2026-08-15)

2026-08-12부터 백로그로 남아있던 항목("investor-panel만 커버가 2장, 다른 상품은 3장") 해소. 기존 2장은 초창기(2026-08-07) 실제 Notion 화면 스크린샷이라 나머지 21종의 통일된 브랜드 목업 스타일(보라 그라디언트 히어로/다크 카드 페르소나/라벤더 데모 카드)과 톤이 달랐음 — 2장 전부 교체하고 신규 3장(hero/personas/demo)으로 맞춤. Cloudflare Browser Rendering으로 렌더링 → `investor-panel-exhibits/`에 커밋 → Gumroad `DELETE .../covers/:id` 2회로 구커버 제거 → `POST .../covers`(raw.githubusercontent.com) 3회로 신규 등록 → `GET`으로 3장 확인 + 라이브 페이지 og:image 갱신 확인까지 완료. 다른 필드(가격/설명/태그/receipt/offer_code)는 이상 없어 손대지 않음.

## Notion 워크스페이스 이원화 발견 (2026-08-15, 회장 확인 필요)

이 세션에 연결된 Notion MCP가 이 상품을 실제로 만든 계정(`tossneon0`, `fearless-frog-802.notion.site`)이 아니라 **다른 워크스페이스**("나다컴퍼니 > 신사업 탐색 > 🏪 상품 허브")에 물려 있다는 걸 확인했다. 그 워크스페이스 안에도 Investor Panel·Code Review Board를 포함한 20종 템플릿이 전부 완성된 상태로 존재해서 처음엔 "회장 개인 계정에 있는 중복 사본"으로 추정했으나, **회장이 채팅으로 확인한 결과는 다름** — 이 워크스페이스는 회장이 새로 만든 **이관 목적지**이고, 구 워크스페이스(`tossneon0`) 자료를 옮기는 중이며 "자리잡으면 기존거 지울게"로 확정. 즉 혼란이 아니라 진행 중인 정식 이관. `candidates.md`의 2026-08-13 "Investor Panel 재발견" 항목은 이 이관 중 사본을 가리킨 것이었을 뿐, 새 사업기회는 아니었음 — 정정 기록 및 **구 워크스페이스 삭제 전 22개 상품 receipt 링크 교체 필요** 후속 항목은 `candidates.md` 참고.
