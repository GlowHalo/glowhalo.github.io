 ...
 *
 * 비밀값은 코드에 두지 않는다. 정본은 `tossneon-api-vault`(계정 공용 금고, 유일한 창고) —
 * 값을 그 금고에서 조회한 뒤 `wrangler secret put`으로 이 Worker에 등록한다.
 * 자세한 절차는 `.claude/rules/cloudflare-vault.md` 참고.
