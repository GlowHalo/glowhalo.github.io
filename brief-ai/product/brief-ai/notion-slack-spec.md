# Notion/Slack 자동 전송 — 기능 스펙 (2026-08-17 초안 → 같은 날 V1 골격 구현 완료)

결제 연동이 회장 판단으로 보류 중인 동안, 실제 결제 없이도 설계·의존관계 파악까지는 지금 끝내둔다. 결제가 풀리는 즉시 아래 순서대로 바로 착수 가능하도록 남긴 문서.

**✅ 2026-08-17 같은 날 구현·배포·실측 완료**: 아래 "V1" 설계 그대로 이메일 매직링크 인증(`/v1/auth/request`·`/v1/auth/verify`), 설정 화면(`/settings`), 연동 저장(`/v1/settings/integrations`, AES-256-GCM 암호화), 전송 로직(`sendToNotion`/`sendToSlack`)까지 구현해 라이브에 배포했고 전체 흐름 실측 완료(`README.md` 테스트 로그 참고). **유일하게 남은 건 결제 웹훅 하나뿐** — 웹훅 수신 시 `USERS_KV`의 `user:<email>.plan`을 `"pro"`로 갱신하는 코드 몇 줄만 추가하면 지금 이미 배포된 전송 로직이 그대로 발동한다.
