...
- 두 경우 다 "노션 동기화해줘"라고 해야 허브 화면에 실제 반영됨(동기화 → 커밋 → push. 확인 기준은 루트 `CLAUDE.md`의 "자율성과 확인 원칙" 참고).
...
4. 저장소 루트에서 `git add` + `git commit` + `git push` — 저장소가 하나뿐이라 push 한 번이면 허브·프로젝트가 전부 백업되고 1분 내 배포됨 (확인 기준은 루트 `CLAUDE.md`의 "자율성과 확인 원칙" 참고)
