...
- **2026-08-08 추가 시도 — 헤드리스 브라우저 로그인 자동화, 재확인된 구조적 차단.** 금고에 등록된 `gumroad_login_email`/`gumroad_login_password`로 Playwright(Chromium) 자동 로그인을 시도했으나, **이 세션의 아웃바운드 에이전트 프록시와 Chromium의 TLS 핸드셰이크가 호환되지 않는 문제**(`net::ERR_CONNECTION_RESET`)로 `https://gumroad.com/login` 페이지 자체를 열지 못했다. 이전 세션이 [06-code-review-board.md](products/06-code-review-board.md) 부록에서 이미 보고한 것과 동일한 현상을 오늘 독립적으로 재확인:
  ...
  - 결론: 이 계정 레벨 환불정책 활성화 스위치는 **이 세션 환경에서는 브라우저 자동화로 켤 수 없다.** 아래 "회장님이 하실 일"의 대안 경로(회장이 직접 1회 확인)가 사실상 유일한 다음 단계다.
...
1. ~~Gumroad 로그인 정보를 금고에 등록~~ ✅ 완료(이메일·비밀번호 모두 금고에 등록됨). 하지만 **2026-08-08 확인 결과, 이 정보를 써도 이 세션 환경에서는 브라우저 자동화로 로그인 자체가 불가능**하다(위 "환불정책 조사 결과"의 추가 시도 참고 — 세션 프록시와 Chromium TLS 핸드셰이크 비호환, `curl`/Node API 호출은 정상인데 Chromium만 막힘). 즉 로그인 정보 등록은 끝났지만, 그것만으로는 이 막힘이 풀리지 않는다.
2. **남은 유일한 실질 경로: 회장이 직접 아래 링크를 열어 "계정 레벨 환불정책" 스위치를 1회 켜주는 것.** 이후 수치 조정(7일/30일 등)은 API로 사장이 처리 가능하다.
   - 로그인: https://app.gumroad.com/login
   - 환불정책 설정 위치: Settings → Payments 안에 "Refund policy" 섹션(계정 커스텀 환불정책 토글) — 정확한 URL은 로그인 후 좌측 사이드바 Settings에서 Payments 탭을 열면 확인 가능.
   - 켠 뒤에는 별도 조치 불필요 — 사장이 `PUT /v2/refund_policy`로 7일 값을 넣고 `GET`으로 재검증하겠다.
