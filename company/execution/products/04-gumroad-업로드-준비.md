...
6. **채널 자동화 가능 여부 확인 ([원칙 6](../../README.md#운영-원칙) 신규 적용)** — Gumroad는 공식 REST API 보유(Settings → Advanced → Applications에서 access token 발급). 토큰 발급되면 사장이 직접 API로 리스팅 등록 시도 → 되면 이후 모든 등록·수정은 회장 개입 없이 사장이 전담. 안 되면(샌드박스 네트워크 제약 등) 회장이 직접 업로드하는 방식으로 폴백, 그 경우도 이 문서에 결과 기록.
7. 사장이 리스팅 등록 (계정·정산계좌 완료되면 바로 진행)
