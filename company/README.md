...
**(2026-08-08 개편)** 회장이 틈틈이 들여다볼 수 있는 시각화 화면은 이제 **나다그룹 HQ** — `../nada-group/` (배포 후 `https://tossneon.github.io/nada-group/play/`). 예전에 따로 있던 "작업실 컴퍼니"(이 문서의 옛 이름), 저장소 루트의 `ai-office/` 상태판, `pixel-ai-office`의 "새사업 스튜디오"는 전부 이 구조 하나로 합쳐졌다:

- **나다그룹**: HQ 진입 화면. 관계사(계열사) 카드 목록을 보여준다. 지금은 나다컴퍼니1 하나.
- **나다컴퍼니1(신사업)**: 이 문서(`company/`)가 다루는 회사의 새 이름. 정연 사장이 이끄는 탐색 전담 관계사 — 카드를 클릭하면 픽셀 오피스 시뮬레이션(부서 12개, 직원들이 돌아다니며 일하는 화면)으로 들어간다. 예전 `ai-office/status.js`처럼 손으로 갱신하는 별도 상태판은 없어졌고, 이 픽셀 오피스 자체가 그 역할을 대신한다.

계열사가 늘어나면 `nada-group/src/HqApp.tsx`의 관계사 목록에 카드를 추가하는 식으로 확장한다.
