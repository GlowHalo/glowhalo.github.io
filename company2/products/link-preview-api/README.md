...
6. **다만 최초 매직링크는 놓쳤다** — 포워딩 설정 *이전*에 발송된 메일이라 소급 전달이 안 됐다(Gmail MCP로 검색해봐도 안 잡힘, 옆으로 새 확인용 `webshare` 인증메일은 정상 검색돼서 검색 자체는 문제없음을 확인). 새 링크를 받으려고 재가입·재로그인·비밀번호 재설정을 몇 차례 시도했으나: 재가입은 "Username and email must be unique"로 막히고(계정이 이미 생성돼 있음), 로그인은 "Incorrect username or password"만 뜨고(미인증 계정 공통 에러로 추정), 마지막 시도에서는 로그인 폼 자체가 안 잡히는 현상까지 나타나 **짧은 시간에 반복 시도한 게 RapidAPI 쪽 봇 탐지에 걸렸을 가능성**이 있어 더 밀어붙이지 않고 중단함.
7. **다음 단계(회장 액션 또는 시간을 두고 재시도)**: (a) 회장이 실제 브라우저로 RapidAPI에 직접 로그인 시도해보면 "이메일 인증 필요, 재발송" 같은 더 명확한 안내가 뜰 가능성이 높음(자동화 브라우저와 달리 봇 탐지에 안 걸림), 또는 (b) 시간을 좀 두고(예: 몇 시간 뒤) 하윤이 재시도.
8. 가입 완료되면 Platform REST API로 `openapi.json` 업로드해 리스팅 생성.
9. 리스팅 후 `RAPIDAPI_PROXY_SECRET` 발급받아 시크릿 등록, 게이트웨이 경유 호출만 허용하도록 잠금.
10. 실제 RapidAPI 테스트 콘솔로 종단 검증.
