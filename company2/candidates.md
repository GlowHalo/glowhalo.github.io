...
2. **C1(쿠팡파트너스) 실행안 구체화** — "가격 추적/특가 알림 봇" 컨셉을 다음 라운드에서 더 다듬고, 무자본 검증 가능하면 바로 가입·프로토타입 착수.
