...
4. **기록은 이 폴더에 남긴다.** Notion이 아니라 `biz-exploration/`가 경영 의사결정 기록의 단일 소스. 회의록은 `board/`, 상신 안건은 `proposals/NNN-제목.md`, 실제 실행 작업은 `execution/`에 남긴다.
...
- **나다컴퍼니1(신사업)**: 이 문서(`biz-exploration/`)가 다루는 회사. HQ 콘솔 안에서 **직접 운영 화면(mode: "op")**으로 보여준다 — 대표실(정연)·전략팀(CSO)·기술팀(CTO)·그로스팀(CMO)·회의실 방, `biz-exploration/candidates.md`의 A1~A5 사업 라인, 승인 대기·지시 접수함까지. 데이터 소스는 `nada-group/src/data/holdco.config.ts` — `registry.js`와 같은 **스냅샷 패턴**(실시간 연동 아님, 이 문서·`candidates.md`에 의미 있는 변화가 있을 때마다 갱신). **주의**: 이 콘솔은 임원진 실명(혜안·재현·윤슬)을 의도적으로 숨기고 역할명만 노출한다 — 이 문서의 실명 표기를 고친 게 아니라 화면 표시 정책만 그런 것.
...
| 나다컴퍼니11 신설 | 신사업 검토 계열사 — 나다컴퍼니9보다 더 급진적인 방법론: 20인 전문가가 기존 내역 무시하고 각자 아이디어 제안 → 교차 토론 → **최다 지지를 받은 아이디어의 제안자가 대표로 취임**해 그 사업을 직접 이끔 | ✅ 대표 "이든" 선임(2026-08-12). **Round 1 완료** — 20인 전문가 독립 제안 → 교차 투표(20표 중 14표 획득)로 "AI 회의록 자동정리·액션아이템 트래커 SaaS"(사업명 "브리프AI")가 압도적 승리. 문서는 [`biz-review/`](../biz-review/README.md), 투표 결과는 [`biz-review/candidates.md`](../biz-review/candidates.md) |
